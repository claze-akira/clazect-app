...
# ===== 会社マスタ =====
def load_companies(sh):
    try:
        ws = sh.worksheet('会社マスタ')
        data = ws.get_all_records()
        return [r['会社名'] for r in data if r.get('会社名')]
    except:
        return []

def save_companies(sh, companies):
    df = pd.DataFrame({'会社名': companies})
    try:
        ws = sh.worksheet('会社マスタ')
        ws.clear()
    except gspread.WorksheetNotFound:
        ws = sh.add_worksheet(title='会社マスタ', rows=100, cols=5)
    ws.update([['会社名']] + [[c] for c in companies])
...
def parse_csv(uploaded_file):
    ...
    if csv_type == 'unknown':
        return None, '対応していないCSV形式です（freeeまたはMFクラウドの仕訳帳CSVを使用してください）'
    ...
    if csv_type == 'freee':
        ...
        iCrDept=gi('貸方部門'); iPartner=gi('借方取引先名'); iNote=gi('取引内容')
    else:  # mf
        iDate=gi('取引日'); iDkAcc=gi('借方勘定科目'); iDkAmt=gi('借方金額(円)')
        iDkDept=gi('借方部門'); iCrAcc=gi('貸方勘定科目'); iCrAmt=gi('貸方金額(円)')
        iCrDept=gi('貸方部門'); iPartner=gi('借方取引先'); iNote=gi('摘要')
    ...
    for line in lines[1:]:
        ...
        partner = r[iPartner].strip() if iPartner >= 0 and iPartner < len(r) else ''
        ...
        for sheet_name in wb.sheetnames:
            ...
            for row in ws.iter_rows(min_row=header_row+1):
                ...
                for col, month in month_cols.items():
                    ...
                    if val == 0:
                        continue
                    if label == '売上高': bud_type = '売上高'
                    elif label == '仕入高': bud_type = '仕入高'
                    else: bud_type = '費用'
...
col_title, col_company = st.columns([2, 2])
with col_title:
    st.title('📈 業績分析表')
with col_company:
    if companies:
    ...
    if jn_file and sel_company:
        with st.spinner('取込中...'):
            ...
            if err:
                ...
                st.warning('取込対象のデータがありませんでした')
            else:
                try:
                    sh = get_spreadsheet()
                    save_to_sheet(sh, f'{sel_company}_実績データ', df_new)
    ...
    if bud_file and sel_company:
        with st.spinner('取込中...'):
            ...
            if err:
                ...
                try:
                    sh = get_spreadsheet()
                    save_to_sheet(sh, f'{sel_company}_予算データ', bdf_new)
                    st.success(f'✅ 予算データを保存しました')
                    st.cache_data.clear()
                except Exception as e:
                    st.error(f'保存エラー: {e}')
...
with tab_settings:
    ...
    if current_companies:
        for i, c in enumerate(current_companies):
        ...
        st.info('会社が登録されていません')

    st.divider()
    st.write('**会社を追加**')
    new_company = st.text_input('会社名', placeholder='例：株式会社〇〇')
    if st.button('追加', type='primary'):
        if new_company and new_company not in current_companies:
            ...
            st.rerun()
        elif not new_company:
            st.warning('会社名を入力してください')
        else:
            st.warning('すでに登録されている会社名です')
